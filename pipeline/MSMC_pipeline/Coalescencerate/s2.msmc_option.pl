...
foreach my $key (keys %pop_mask){
   ...
   foreach my $key2 (keys %pop_mask){
       ...
       #==========run rate===============
       if($pop_size==1){
           print S2 "cd $indir;/ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/history/MSMC/msmc -R --skipAmbiguous -P '0,0,1,1' -o $a1.outfile -i 50 -t 10 -p '15*1+15*2' $sum_txt\n";
       }
       elsif($pop_size==2){
           print S2 "cd $indir;/ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/history/MSMC/msmc -R --skipAmbiguous -P '0,0,0,0,1,1,1,1' -o $a1.outfile -i 50 -t 10 -p '15*1+15*2' $sum_txt\n";
       }
       else{
           die "warning: the haplotypes estimating Coalescence_rate of MSMC should not be more than 8 haplotype\n";
       }
