...
my $gunbin = "/ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/gnuplot-4.6.0/bin/gnuplot";
...
set y2ran [-52:-20]
#set y2label "MAR (g/cm3/1,000 years),Global sea level (10m),Surface air temperature (0C) " $afont rotate by 270;
set y2label "NGRIP .18O %,EDML .18O %" $afont rotate by 270;
  ...
  set style line 100 lt -1 lc rgb "purple" lw 3;
  set style line 101 lt -1 lc rgb "orange" lw 3;
  set style line 102 lt -1 lc rgb "grey" lw 3;
...
my $zwj_i=2;  
  if ( $opts{M}) {
          ...
          #print $fh qq(;\n);
          print $fh qq(,"North_Greenland_Ice_Core_Project_short.txt" u 1:2 t "NGRIP .18O %" w st ls 100 axes x1y2);
          #print $fh qq(,"NGRIP_ice_core.txt" u 1:2 t "NGRIP temperature, oC" w st ls 101 axes x1y2);
          print $fh qq(,"EDML_ice_core_short.txt"  u 1:2 t "EDML .18O %"  w st ls 102 axes x1y2;\n);
