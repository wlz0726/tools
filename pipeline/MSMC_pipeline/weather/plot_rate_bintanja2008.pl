...
set y2label "Global sea level (10m),Surface air temperature (0C) " $afont rotate by 270;
#  set out "$prefix.eps";
  ...
  set style line 100 lt 13 lc rgb "grey" lw 3;
  set style line 101 linetype 2 lc rgb "grey" lw 3;
  set style line 102 linetype 3 lc rgb "grey" lw 3;
...
my $zwj_i=2;  
  if ( $opts{M}) {
          ...
          #print $fh qq(;\n);
          print $fh qq(,"bintanja2008_wendu.txt" u 1:2 t "Global sea level (10m)" w st ls 101 axes x1y2, "bintanja2008_level.txt"  u 1:2 t "Surface air temperature (0C)"  w st ls 102 axes x1y2;\n);
