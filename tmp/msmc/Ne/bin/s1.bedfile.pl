...
while(<IN>){
    chomp;
    my ($sample,$bam,$bool,$ave_depth)=(split)[0,1,2,3];
...
date
echo start
\$samtools mpileup -q 20 -Q 20 -C 50 -u -r $chr -P ILLUMINA -f \$ref $bam|\$bcftools view -cgI - |\$python \$tools/bamCaller.py $ave_depth $sam_outdir/$sample.$chr.mask.bed > $sam_outdir/$sample.$chr.raw.vcf
date
echo end
Script
        print T "$sh\n$sh2\n";
...
#==============bam including all chromosome=========    
    foreach my $chr(sort keys %chrome){
        open T,">$sam_outdir/msmc_bed_$chr.sh" or die $!;
#        `ln -s /ifshk5/PC_HUMAN_EU/USER/zhuwenjuan/work/Yao90/history/new_MSMC/Ne/bin/1KG/chr$chr.mask.bed $sam_outdir/$sample.$chr.mask.bed`;
my $sh2=<<Script;
cd $sam_outdir
date
echo start
...
foreach my $key (sort keys %sum_vcf){    
    print TXT "cd $outdir/MSMCinputfile;export LD_LIBRARY_PATH=/opt/blc/python-3.1.2/lib:\$LD_LIBRARY_PATH;/opt/blc/python-3.1.2/bin/python3.1 /ifshk5/PC_HUMAN_EU/USER/zhuwenjuan/bin/software/history/MSMC/msmc-master/tools/generate_multihetsep.py $sum_mask{$key} $sum_vcf{$key} > $outdir/MSMCinputfile/msmc.$key.txt\n";
}
close BED;
close TXT;
close IN;   
