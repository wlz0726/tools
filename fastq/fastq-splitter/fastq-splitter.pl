...
sub split_file
{
    ...
    if ($part_size)
    {
        ...
        open(my $IN,'<','zcat $infile|') or die "Error: Can't open file \"$infile\"\n";
        my $part = 1;
        my $this_part_size = 0;
        my $num_len = length($n_parts_measured);
        my $out_file = sprintf("%s.part-%0*d%s",$base,$num_len,$part,$ext);
        open(my $OUT,'|gzip -c > $out_file.gz') or die "Can't create output file \"$out_file\"\n";
        binmode $OUT;
        while (!eof $IN)
        {
            ...
            if ($this_part_size and ($this_part_size + $entry_size > $part_size))
            {
                ...
                open ($OUT,'|gzip -c > $out_file.gz') or die "Can't create output file \"$out_file\"\n";
                binmode $OUT;
                $this_part_size = 0;
            }
            print $OUT $entry;
...
{
    my ($file) = @_;
    open(my $IN,'zcat $file|') or die "Error: Can't open file \"$file\"\n";
