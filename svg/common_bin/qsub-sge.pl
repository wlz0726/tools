...
(4) We provide a "--reqsub" option, to throw the unfinished jobs automatically, until all the jobs are 
	...
	reqsub cycle number allowed is 10000.
...
=head1 Usage
  ...
  --interval <num>  set interval time of checking by qstat, default 50 seconds
  ...
  --project<str>    set the prefix for  project 
...
my ($Queue, $Interval, $Lines, $Maxjob, $Convert,$Secure,$Reqsub,$Resource,$Job_prefix,$Project,$Verbose, $Help);
GetOptions(
	"lines:i"=>\$Lines,
	"maxjob:i"=>\$Maxjob,
	"interval:i"=>\$Interval,
...
while (@Shell) {
	...
	foreach my $job_id (sort keys %Alljob) {
		...
		if (-f "$shell_file.o$job_id") {
			chomp (my @lines = `less -S $shell_file.o$job_id`);
			@lines = grep (/This-Work-is-Completed!/, @lines);
			my $line_num = (split (/\s+/,`wc -l $shell_file`)) [0];
			...
			elsif ($line_num > @lines) {
				$Error{$job_id} = $shell_file;
				my $unfinished_line_num = $line_num - @lines;
				print OUT "In qsub cycle $qsub_cycle, In $shell_file.o$job_id,  $unfinished_line_num \"This-Work-is-Completed!\" is not found, so this work may be unfinished\n";
			}
		}
#		if (-f "$shell_file.o$job_id") {
...
#		}
		...
		##check whether blastall can be found
		if ($content =~ /blastall: No such file or directory/) {
			$Error{$job_id} = $shell_file;
			print OUT "In qsub cycle $qsub_cycle, In $shell_file.e$job_id, blastall is not found, so this work may be unfinished\n";
		}

		##check whether treebest run normally
		if ($content =~ /Segmentation fault.*treebest/) {
			$Error{$job_id} = $shell_file;
			print OUT "In qsub cycle $qsub_cycle, In $shell_file.e$job_id, treebest: failed, so this work may be unfinished\n";
		}

		##check whether run normally of other software
		if ($content =~ /Floating point exception/) {
			$Error{$job_id} = $shell_file;
			print OUT "In qsub cycle $qsub_cycle, In $shell_file.e$job_id, exist \"Floating point exception\", so this work may be unfinished\n";
		}
	}
...
sub absolute_path{
	...
	while (<IN>) {
		...
		for (my $i=0;$i<@words ;$i++) {
			if (($words[$i]!~/^\//) && ($words[$i]=~/\//) && ($words[$i]!~/PATH=".*\$PATH.*"/) && ($words[$i]!~/PERLLIB=".*\$PERLLIB.*"/) && ($words[$i]!~/WISECONFIGDIR=/) && ($words[$i]!~/AUGUSTUS_CONFIG_PATH=/)) {
