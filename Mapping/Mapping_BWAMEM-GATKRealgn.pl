...
foreach my $read1(@read1){
    ...
    print O "export JAVA_HOME=$javahome; $java -Xmx5g -jar $picard MarkDuplicates INPUT=$outdir/$id.sort.bam OUTPUT=$outdir/$id.rmdup.bam METRICS_FILE=$outdir/$id.dup.txt REMOVE_DUPLICATES=true; $samtools index $outdir/$id.rmdup.bam;  ";
