...
while(<I>){
    ...
    my $id2=$2;
    `mkdir $outdir/$id1`;
    my $read1="$outdir/$id1/$id2.fastq.gz";
    ...
    print O2 "$bwa mem -t 30 -M -R \'\@RG\\tID:$name\\tLB:$name\\tSM:$name\\tPL:Illumina\\tPU:Illumina\\tSM:$name\\t\' $ref $read1 |$samtools sort -O bam -T $outdir/tmp/$id2 -o $outdir/$id1/$id2.sort.bam; $samtools index $outdir/$id1/$id2.sort.bam;  ";
    print O2 "$java -Xmx5g -jar $picard MarkDuplicates INPUT=$outdir/$id1/$id2.sort.bam OUTPUT=$outdir/$id1/$id2.rmdup.bam METRICS_FILE=$outdir/$id1/$id2.dup.txt REMOVE_DUPLICATES=true; $samtools index $outdir/$id1/$id2.rmdup.bam;  ";
