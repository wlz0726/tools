...
#tongji
if (@ARGV !=4) {
	print "perl $0 <poplist><pop-pair><chrlist><outdir>\n";
	exit 0;
}
my ($poplist,$pop_pair,$chrlist,$outdir)=@ARGV;
open IN,$poplist or die $!;
open PAIR,$pop_pair or die $!;
my %intersting_pair=();
while(<PAIR>){
    chomp;
    my ($pop1,$pop2)=(split(/\:/,$_))[0,1];
    my $key="$pop1"."_$pop2";
    $intersting_pair{$key}=1;
    $key="$pop2"."_$pop1";
    $intersting_pair{$key}=1;
}
close PAIR;
...
foreach my $key (keys %pop_mask){
   ...
   foreach my $key2 (keys %pop_mask){
       ...
       if($intersting_pair{$a1}){;} elsif($intersting_pair{$a2}){;}else{next;} #only intersted some population pairs.
       if($have{$a1}){next;}
       elsif($have{$a2}){next;}
       else{$have{$a1}=1;}
       ...
       foreach my $chr (sort keys %chrome){
           ...
           print S1 "export LD_LIBRARY_PATH=/opt/blc/python-3.1.2/lib:\$LD_LIBRARY_PATH;/opt/blc/python-3.1.2/bin/python3.1 /ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/history/MSMC/msmc-master/tools/generate_multihetsep.py $sum_mask_bed > $indir/$a1/msmc.$chr.txt \n";
       }
       #==========run rate===============
       if($pop_size==1){
           print S2 "/ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/history/MSMC/msmc-master/build/msmc -R --skipAmbiguous -P '0,0,1,1' -o $indir/$a1/$a1.outfile -i 50 -t 10 -p '15*1+15*2' $sum_txt\n";
       }
       elsif($pop_size==2){
           print S2 "/ifshk5/BC_COM_P11/F16RD04012/ICEmmrD/bin/software/history/MSMC/msmc-master/build/msmc -R --skipAmbiguous -P '0,0,0,0,1,1,1,1' -o $indir/$a1/$a1.outfile -i 50 -t 10 -p '15*1+15*2' $sum_txt\n";
       }
       else{
           die "warning: the haplotypes estimating Coalescence_rate of MSMC should not be more than 8 haplotype\n";
       }
