...
#        `ln -s /ifshk5/PC_HUMAN_EU/USER/zhuwenjuan/work/Yao90/history/new_MSMC/1KG_Coalescence_rate_8haplotype/bin/1KG_bed/chr$chr.mask.bed $sam_outdir/$sample.$chr.mask.bed`;
#        `ln -s /ifshk5/PC_HUMAN_EU/USER/zhuwenjuan/work/Yao90/history/new_MSMC/Coalescence_rate/bin/1KG/chr$chr.mask.bed $sam_outdir/$sample.$chr.mask.bed`;
        open T,">$sam_outdir/msmc_bed_$chr.sh" or die $!;
my $sh2=<<Script;
\$samtools mpileup -q 20 -Q 20 -C 50 -u -r $chr -P ILLUMINA -f \$ref $bam|\$bcftools view -cgI - |\$python \$tools/bamCaller.py $ave_depth $sample.$chr.mask.bed > $sample.$chr.raw.vcf
Script
